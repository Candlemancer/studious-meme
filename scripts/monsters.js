    ...
    updateCharts() {
        ...
        if (this.axisOptions.map(v => v.json).includes(changedValue)) {
            ...
            let bigXScale = d3.scaleLinear()
                .range([0, (this.svgBounds.width / 2) - this.margin.left - this.margin.right])
                .domain([d3.min(
            ...
            bigGraphSVG.append('g')
                .attr('class', 'x-axis')
                .attr('transform', `translate(${yAxisWidth}, ${this.svgBounds.height-xAxisHeight})`)
                .call(bigXAxis)
                .selectAll('text')
            ...
            ;
            // k, I got the axes working and it's late enough I'll put the other stuff in tomorrow
        }
